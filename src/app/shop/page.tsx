...
async function categoryApi() {
  const response = await frontEndHome.categoryApi()
...
const page = async () => {
  ...
  const categorydynamic = res
  console.log("res::::::", categorydynamic);
  return (
    <div className="py-[7.649rem] flex flex-col items-center">
      <div className="relative text-center">
        <h1 className="text-[2rem] md:text-[1.8rem] ">Categories</h1>
      </div>
      <div className="grid md:grid-cols-3 lg:grid-cols-4 gap-10 lg:gap-10 w-[80%] lg:w-[90%] xl:w-[95%]">
        {categorydynamic.map((items:{name: string, _id: string, image: string}, i: number) => (
          <Catoo key={i} name={items.name} image={storageUrl+ items.image} id={items._id}  />
        ))}
      </div>
    </div>
  );
